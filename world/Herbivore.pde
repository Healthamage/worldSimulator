...
//config attributes
  ...
  final static int breedingDelay = 10000;
  ...
  final static float alignmentRadius = 80;
  final static float cohesionRadius = 80;
...
//CONSTRUCTOR
  ...
  void setGeneticInheriteSpeedModifier(float modifier){
    ...
    currentGeneticVariance = currentGeneticVariance + modifier;
...
//state methods
  ...
  void breedingUpdate(long deltaTime){
    ...
    if(breedingTimer.expired()){
      ...
      newHerbivore.setGeneticInheriteSpeedModifier((float)(speed - defaultSpeed)*0.5);
...
//METHODS
  ...
  void poop(){
    ...
    if(energy > 50){
      ...
      this.energy -= 30;
      if(random(10) < 2){
        this.energy -= Plant.defaultEnergy;
